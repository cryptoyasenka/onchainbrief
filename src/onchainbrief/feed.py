...
def _render(items: list[FeedItem]) -> str:
    ...
    for it in items:
        ...
        cards.append(
            ...
            f'<img src="{html.escape(it.card)}" alt="{html.escape(it.headline)}" onclick="openLightbox(this.src,this.alt)" style="cursor:zoom-in;">'
            f'</div>'
    ...
    return (
        ...
        "text-align:center"
        "}"
        ".onboarding-card{background:linear-gradient(135deg,rgba(139,92,246,0.08),rgba(6,180,212,0.03));border:1px solid rgba(139,92,246,0.15);border-radius:24px;padding:32px;margin-bottom:48px;box-shadow:0 20px 40px rgba(0,0,0,0.3);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px)}"
        ".onboarding-card h2{font-size:24px;margin:12px 0 16px 0;background:linear-gradient(to right,#ffffff,#d1d5db);-webkit-background-clip:text;-webkit-text-fill-color:transparent}"
        ".onboarding-card p{color:var(--text-muted);font-size:14px;line-height:1.6;margin-bottom:24px}"
        ".onboarding-badge{display:inline-block;background:var(--primary-glow);border:1px solid rgba(139,92,246,0.3);color:#c084fc;padding:6px 12px;border-radius:999px;font-size:11px;font-weight:600;letter-spacing:0.05em}"
        ".steps-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:24px;border-top:1px solid rgba(255,255,255,0.06);padding-top:24px}"
        ".step-item{display:flex;gap:16px}"
        ".step-num{font-family:monospace;font-size:24px;font-weight:700;color:var(--accent);text-shadow:0 0 10px rgba(6,182,212,0.3);line-height:1}"
        ".step-item strong{display:block;font-size:14px;color:#ffffff;margin-bottom:6px}"
        ".step-item p{font-size:12px;margin:0;line-height:1.5}"
        ...
        "<div class=onboarding-badge>⚡ SECURITY AUTONOMOUS INTEL</div>"
        "<h2>Real-time Solana Threat & Activity Intelligence</h2>"
        "<p>OnchainBrief is an autonomous agent that continuously monitors the Solana blockchain, identifies notable program activities, and generates verifiable intelligence reports. Each card represents a signed threat/activity brief.</p>"
        "<div class=steps-grid>"
        "<div class=step-item><span class=step-num>01</span><div><strong>Surveillance</strong><p>Monitors contract deployments, program upgrades, slippage anomalies, and compute fee trends.</p></div></div>"
        "<div class=step-item><span class=step-num>02</span><div><strong>Synthesis</strong><p>Queries context, extracts narratives, and designs high-tech visual trading cards.</p></div></div>"
        "<div class=step-item><span class=step-num>03</span><div><strong>Provenance</strong><p>Anchors the signature on-chain and attests origin via Sol-Agent Protocol (SAP).</p></div></div>"
        "</div>"
        "</section>"

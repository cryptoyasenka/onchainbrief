...
def _render(items: list[FeedItem]) -> str:
    ...
    return (
        ...
        ".hero{text-align:center;margin-bottom:56px;max-width:800px;margin-left:auto;margin-right:auto}"
        ...
        "</nav>"
        "<section class=hero>"
        "<div class=hero-badge>⚡ SECURITY SURVEILLANCE</div>"
        "<h1>Autonomous Solana Security & Activity Intelligence</h1>"
        "<p class=hero-desc>OnchainBrief triages complex real-time blockchain logs into clean, verified threat and deployment briefs, anchored on-chain for complete provenance.</p>"
        "</section>"
        "<section class=value-grid>"
        "<div class='value-card v1'>"
        "<div class=value-question>WHAT IS ONCHAINBRIEF?</div>"
        "<h3>Real-time Network Surveillance</h3>"
        "<p>An autonomous AI agent that continuously monitors the Solana ledger. It triages millions of transactions to capture critical developer actions, program upgrades, deployments, and fee surges.</p>"
        "</div>"
        "<div class='value-card v2'>"
        "<div class=value-question>WHY DOES IT EXIST?</div>"
        "<h3>Factual Curation in the Noise</h3>"
        "<p>On-chain logs are complex, spam-heavy, and unreadable. The agent filters the noise, fetches real-time web context, and synthesizes clean, no-hype security briefs with source references.</p>"
        "</div>"
        "<div class='value-card v3'>"
        "<div class=value-question>WHAT VALUE DOES IT BRING?</div>"
        "<h3>Verifiable Ledger Provenance</h3>"
        "<p>Trust is built-in. Every brief is cryptographically signed and attested to Solana's ledger via Memo & SAP protocol, producing a permanent, tamper-proof history of network actions.</p>"
        "</div>"
        "</section>"
